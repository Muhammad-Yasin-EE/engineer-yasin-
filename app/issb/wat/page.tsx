        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-3 bg-slate-900/90 border border-slate-800 py-2 px-4 rounded-full shadow-inner">
              ...
              <span className="text-xs font-black uppercase tracking-wider text-gray-300">Official Armed Forces Testing Hub</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2 text-xs font-black uppercase tracking-wider text-gray-300">
            <span className="px-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-emerald-400 flex items-center gap-1.5 shadow-sm">
              <Clock className="w-4 h-4 text-emerald-400 shrink-0" /> 10 Seconds Per Stimulus Word
            </span>
            <span className="px-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-[#D4AF37] flex items-center gap-1.5 shadow-sm">
              <Award className="w-4 h-4 text-[#D4AF37] shrink-0" /> 100 Words Complete Test
            </span>
            <span className="px-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 flex items-center gap-1.5 shadow-sm">
              <ShieldCheck className="w-4 h-4 text-slate-300 shrink-0" /> Official Military Selection Standard
            </span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-10 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl">
          <div className="relative w-full sm:w-96">
            ...
            <input
              type="text"
              placeholder="Search test number (e.g. Test 01)..."
              value={searchQuery}
          ...
          <div className="text-xs font-black text-gray-400 uppercase tracking-widest w-full sm:w-auto text-center sm:text-right">
            Showing <span className="text-[#D4AF37] font-bold">{filteredSets.length}</span> of 15 Tests
          </div>
        </div>

        {/* ── RESPONSIVE OFFICIAL MILITARY CARD GRID ── */}
        {filteredSets.length === 0 ? (
          <div className="p-16 bg-slate-900/50 border border-slate-800 rounded-3xl text-center text-gray-400 text-base font-medium">
            No test matches your search query. Try searching for a number from 01 to 15.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSets.map((set, i) => {
              const testNumber = (i + 1).toString().padStart(2, '0')
              return (
                <div
                  key={set.id}
                ...
                >
                  <div className="space-y-4 relative z-10">
                    <div className="flex items-center justify-between gap-2">
                      <span className="px-3.5 py-1.5 rounded-xl bg-slate-950 text-[#D4AF37] font-black text-xs uppercase tracking-widest border border-[#233554] shadow-md shrink-0">
                        Test #{testNumber}
                      </span>
                      <span className="px-3 py-1 bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 font-extrabold text-[11px] rounded-full uppercase tracking-wider shrink-0">
                        100 Words
                      </span>
                    </div>
                    
                    <div className="space-y-1.5 pt-1">
                      <h3 className="text-xl sm:text-2xl font-black text-white group-hover:text-emerald-400 transition-colors leading-snug">
                        WAT Test {testNumber}
                      </h3>
                      <span className="text-xs font-bold text-gray-400 block">
                        Official Armed Forces Psychological Test
                      </span>
                    </div>
                    ...
                    <div className="pt-2 flex items-center gap-2 flex-wrap text-[11px] font-extrabold">
                      <span className="px-3 py-1 bg-slate-950/80 rounded-lg text-slate-300 border border-slate-800 flex items-center gap-1.5">
                        ⚡ 100 Words Test
                      </span>
                      <span className="px-3 py-1 bg-slate-950/80 rounded-lg text-emerald-300 border border-slate-800 flex items-center gap-1.5">
                        ⏱️ 10s Slide Interval
                      </span>
                    </div>
                  </div>
