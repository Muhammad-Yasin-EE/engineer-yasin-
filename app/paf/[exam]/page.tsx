...
export async function generateMetadata({ params }: { params: Promise<{ exam: string }> }) {
  ...
  return baseMeta({ params: Promise.resolve({ category: 'paf', exam }) })
}

export default async function PafExamPage({ params }: { params: Promise<{ exam: string }> }) {
  const { exam } = await params
  return <ExamPage params={Promise.resolve({ category: 'paf', exam })} />
}
